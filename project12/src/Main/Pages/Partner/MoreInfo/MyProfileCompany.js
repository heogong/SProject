...
import { Alert, StyleSheet, ScrollView, TouchableOpacity, View, Image } from 'react-native';
...
class MyProfileCompany extends Component {
	...
	_editProdSpecialty = () => {
		EditProdSpecialty(ARRAY_SELECT_SPECIALTY).then((result) => {
			GetCommonData(result, this._editProdSpecialty).then(async (resultData) => {
				console.log('업체 취급 제품 유형 목록 수정 - ', resultData);
				if (resultData !== undefined) {
					Alert.alert(
						'',
    ...
    if(workData[dayKey] == 'Y') {
      BUSINESS_DAY[dayKey] = 'Y';
      return true;
    } else {
      return false;
    }
							...
							<View>
								...
								<View>
									<View style={[ styles.fxDirRow, styles.alignItemsCenter ]}>
										...
										>
											<Text style={localStyles.timeTxt}>{`${this.state.stHour}:${this.state
												.stMin}`}</Text>
										</TouchableOpacity>
										...
										<TouchableOpacity
											...
											style={localStyles.timeTxtWrap}
										>
											<Text style={localStyles.timeTxt}>{`${this.state.edHour}:${this.state
												.edMin}`}</Text>
										</TouchableOpacity>
									</View>
								</View>
								...
								<View style={[ styles.fxDirRow, styles.justiConCenter ]}>
									<View style={[ styles.checkBoxWrap, { marginRight: 8 } ]}>
										{this.state.fullWorkYn ? (
											<TouchableOpacity
												onPress={async () => {
													await this.setState({ fullWorkYn: false }),
														await this._handleFullBtnClick();
												}}
											>
												<Image
													source={require('~/Common/Image/check_circle_on.png')}
													resizeMode="contain"
											...
											<TouchableOpacity
												onPress={async () => {
													await this.setState({ fullWorkYn: true }),
														await this._handleFullBtnClick();
												}}
											>
												<Image
													source={require('~/Common/Image/check_circle_off.png')}
													resizeMode="contain"
													style={styles.checkboxIcon}
												/>
											</TouchableOpacity>
									...
									<View style={[ styles.checkBoxWrap, { marginLeft: 8 } ]}>
										{this.state.holidayWorkYn ? (
											<TouchableOpacity
												onPress={async () => {
													await this.setState({ holidayWorkYn: false }),
														(BUSINESS_DAY.holidayWorkYn = (await this.state.holidayWorkYn)
															? 'Y'
															: 'N');
												}}
											>
											...
											<TouchableOpacity
												onPress={async () => {
													await this.setState({ holidayWorkYn: true }),
														(BUSINESS_DAY.holidayWorkYn = (await this.state.holidayWorkYn)
															? 'Y'
															: 'N');
												}}
