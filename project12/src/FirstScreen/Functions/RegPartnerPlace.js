...
const API_URL = `${DOMAIN}coolinic/partners/addr?`;

function RegPartnerPlaceUrl(bizObj) {
  let addressUrl = `${API_URL}addr.addressName=${bizObj.addressObj.address_name}
  ...
  if(bizObj.addressObj.road_address !== null) {
    let roadAddress = `&road.addressName=${bizObj.addressObj.road_address.address_name}
    ...
    addressUrl = `${addressUrl}${roadAddress}`;
  }

  return addressUrl;
}

const RegPartnerPlace = async (bizObj) => {
  // 토큰값 가져오기
  const ACCESS_TOKEN = `Bearer ${await AsyncStorage.getItem('AccessToken')}`; 
  //console.log("bizObj : ",bizObj);
  //console.log("RegBizPlaceUrl : ",RegBizPlaceUrl(bizObj))

  return fetch(RegPartnerPlaceUrl(bizObj), {
    method: 'POST',
    headers: {
...
export default RegPartnerPlace;